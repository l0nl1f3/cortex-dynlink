...
fn main() {
    let args = Args::parse();
    // list ../testcase
    let mut paths: Vec<_> = std::fs::read_dir("../testcase")
        .unwrap()
        .map(|r| r.unwrap())
        .collect();
    ...
    for case in paths {
        ...
        let o2 = std::process::Command::new("bash")
            .current_dir(&path)
            .arg("-c")
            .arg("cargo build -Zbuild-std=core --release")
            .output()
            .unwrap();
        ...
        let o3 = std::process::Command::new("bash")
            .arg("-c")
            .arg(format!(
                "cp {}  ../build_script/module.o",
                object[0].path().to_str().unwrap()
            ))
            .output()
            .unwrap();
        let o4 = std::process::Command::new("bash")
            .current_dir("../build_script")
            .arg("-c")
            .arg("cargo run")
            .output()
            .unwrap();
